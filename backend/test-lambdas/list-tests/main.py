...
def list_tests(event):
    try:
        offset = event['queryStringParameters']['offset']
        limit = event['queryStringParameters']['limit']

        tests = session.query(Test).offset(offset).limit(limit).all()
        response = { 'tests': [] }

        if len(tests) > 0:    
            for test in tests:
                response['tests'].append(test.as_dict())

        logger.info(f'Found tests: {response}')
